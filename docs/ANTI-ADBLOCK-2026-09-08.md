...
Hash cây file là SHA-256 của JSON danh sách `{path, sha256}`, sắp theo path như runner; không phải hash ZIP. [Release và checksum tải về](https://github.com/TXAVL/ShieldBlock/releases/tag/v1.1.1) dùng các ZIP cục bộ đã kiểm thử trên Chrome ở trên. [Actions](https://github.com/TXAVL/ShieldBlock/actions/workflows/mv3-chromium.yml) kiểm tra nguồn và dựng artifact riêng; không gán hash của build độc lập đó cho ZIP đã thử cục bộ.
