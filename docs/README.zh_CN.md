...
1. 从 [GitHub Releases](https://github.com/TXAVL/ShieldBlock/releases) 下载 `ShieldBlock_*.chromium.zip` 及其对应的 `.sha256` 文件。
...
> [!NOTE]
> 侧载扩展不会通过 Chrome 应用商店更新。请关注 [Releases](https://github.com/TXAVL/ShieldBlock/releases)，并在新版本发布后替换已解压的构建。只安装来自本仓库的构件，并验证随附的 SHA-256 校验和。
...
从浏览器的扩展页面加载 `dist/build/ShieldBlock.chromium`。带版本号的 PowerShell 命令以及可选的带版本号 shell 命令会在 `dist/build/` 下创建 ZIP 和校验和；直接运行 `make mv3-chromium` 只会创建已解压目录。
...
安全问题应通过 [GitHub Security Advisories](https://github.com/TXAVL/ShieldBlock/security/advisories/new) 私下报告，而不是提交公开 issue。报告政策请参阅 [SECURITY.md](../SECURITY.md)。
...
- [提出功能建议或报告错误](https://github.com/TXAVL/ShieldBlock/issues/new/choose)
- [提交 Filter Store 条目](https://github.com/TXAVL/ShieldBlock/issues/new?template=filter_store_submission.yml)
...
ShieldBlock 是基于 [uBlock Origin](https://github.com/gorhill/uBlock) 及从上游继承的 MV3 组件所创作的衍生作品。项目保留版权、源文件头、作者历史和第三方署名。请参阅 [NOTICE.md](../NOTICE.md)。
