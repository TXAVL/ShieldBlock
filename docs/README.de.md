...
1. Laden Sie `ShieldBlock_*.chromium.zip` und die dazugehörige `.sha256`-Datei von [GitHub Releases](https://github.com/TXAVL/ShieldBlock/releases) herunter.
...
> [!NOTE]
> Eine per Sideloading installierte Erweiterung wird nicht über den Chrome Web Store aktualisiert. Folgen Sie den [Veröffentlichungen](https://github.com/TXAVL/ShieldBlock/releases) und ersetzen Sie den entpackten Build, wenn eine neue Version erscheint. Installieren Sie ausschließlich Artefakte aus diesem Repository und prüfen Sie die mitgelieferte SHA-256-Prüfsumme.
...
Laden Sie `dist/build/ShieldBlock.chromium` über die Erweiterungsseite des Browsers. Der versionierte PowerShell-Befehl und der optionale versionierte Shell-Befehl erzeugen ZIP-Datei und Prüfsumme unter `dist/build/`; ein einfaches `make mv3-chromium` erzeugt nur das entpackte Verzeichnis.
...
Sicherheitsprobleme sollten vertraulich über [GitHub Security Advisories](https://github.com/TXAVL/ShieldBlock/security/advisories/new) und nicht in einem öffentlichen Issue gemeldet werden. Die Melderichtlinie finden Sie in [SECURITY.md](../SECURITY.md).
...
- [Eine Funktion vorschlagen oder einen Fehler melden](https://github.com/TXAVL/ShieldBlock/issues/new/choose)
- [Einen Eintrag für den Filter Store einreichen](https://github.com/TXAVL/ShieldBlock/issues/new?template=filter_store_submission.yml)
...
ShieldBlock ist ein abgeleitetes Werk auf Grundlage von [uBlock Origin](https://github.com/gorhill/uBlock) und geerbten MV3-Komponenten des Upstream-Projekts. Urheberrechtsvermerke, Quelltext-Header, Autorenhistorie und Drittanbieterhinweise bleiben erhalten. Siehe [NOTICE.md](../NOTICE.md).
