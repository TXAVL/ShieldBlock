...
1. Obtain the `uBlock-Plus-experimental-webrequest-<commit>` artifact from a successful [Actions run](https://github.com/TXAVL/ShieldBlock/actions/workflows/mv3-chromium.yml), or build it with the commands below. This is a separate artifact from the standard edition. A Git push does not update GitHub Releases.
