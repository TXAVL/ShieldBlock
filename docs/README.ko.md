...
1. [GitHub Releases](https://github.com/TXAVL/ShieldBlock/releases)에서 `ShieldBlock_*.chromium.zip`과 일치하는 `.sha256` 파일을 다운로드합니다.
...
> [!NOTE]
> 사이드로드한 확장 프로그램은 Chrome 웹 스토어를 통해 업데이트되지 않습니다. [Releases](https://github.com/TXAVL/ShieldBlock/releases)를 확인하고 새 버전이 게시되면 압축을 푼 빌드를 교체하세요. 이 저장소의 아티팩트만 설치하고 제공된 SHA-256 체크섬을 검증하세요.
...
브라우저의 확장 프로그램 페이지에서 `dist/build/ShieldBlock.chromium`을 로드하세요. 버전을 지정한 PowerShell 명령과 선택적인 버전 지정 셸 명령은 `dist/build/` 아래에 ZIP과 체크섬을 만듭니다. 일반 `make mv3-chromium`은 압축을 풀어 놓은 디렉터리만 만듭니다.
...
보안 문제는 공개 이슈가 아니라 [GitHub Security Advisories](https://github.com/TXAVL/ShieldBlock/security/advisories/new)를 통해 비공개로 신고하세요. 신고 정책은 [SECURITY.md](../SECURITY.md)를 참고하세요.
...
- [기능 제안 또는 버그 신고](https://github.com/TXAVL/ShieldBlock/issues/new/choose)
- [Filter Store 항목 제출](https://github.com/TXAVL/ShieldBlock/issues/new?template=filter_store_submission.yml)
...
ShieldBlock는 [uBlock Origin](https://github.com/gorhill/uBlock)과 업스트림에서 상속한 MV3 구성 요소를 기반으로 한 파생 저작물입니다. 저작권, 소스 헤더, 작성자 기록, 제3자 저작자 표시는 보존됩니다. [NOTICE.md](../NOTICE.md)를 참고하세요.
