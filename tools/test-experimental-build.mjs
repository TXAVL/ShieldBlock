...
try {
    ...
    if ( process.platform === 'win32' ) {
        ...
        const psCmd = spawnSync('pwsh', [ '-v' ]).error ? 'powershell' : 'pwsh';
        const launch = spawnSync(psCmd, command, { encoding: 'utf8' });
        ...
        const repeat = spawnSync(psCmd, command, { encoding: 'utf8' });
        assert.equal(repeat.status, 0, repeat.stderr);
        assert.equal(await access(specification.profileDirectory).then(() => true, () => false), existed,
            'PrintCommand must not create the isolated profile');
        const forbiddenOverride = spawnSync(psCmd, [ ...command, '-ProfileDirectory', fixture ], { encoding: 'utf8' });
        ...
        for ( const invalid of [
            ...
            const result = spawnSync(psCmd, command, { encoding: 'utf8' });
        ...
        assert.notEqual(spawnSync(psCmd, command, { encoding: 'utf8' }).status, 0,
