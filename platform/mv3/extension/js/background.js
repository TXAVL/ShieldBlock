...
// ShieldBlock - Tab & Lifetime Blocked Counters
const tabBlockedStats = {};
let totalBlockedStats = 0;

localRead('shieldblock_total_blocked').then(val => {
    if ( typeof val === 'number' ) {
        totalBlockedStats = val;
    }
}).catch(() => {});

function recordBlocked(tabId, count = 1) {
    totalBlockedStats += count;
    localWrite('shieldblock_total_blocked', totalBlockedStats).catch(() => {});
    if ( tabId && tabId > 0 ) {
        tabBlockedStats[tabId] = (tabBlockedStats[tabId] || 0) + count;
    }
}

try {
    browser.tabs?.onRemoved?.addListener(tabId => {
        delete tabBlockedStats[tabId];
    });
} catch {}
...
async function onMessage(request, sender) {
    ...
    case 'recordBlocked': {
        ...
        recordBlocked(tabId, request.count || 1);
        return true;
    }
    ...
    case 'popupPanelData': {
        ...
        const pageBlockedCount = (typeof tabBlockedStats !== 'undefined' && tabId && tabBlockedStats[tabId]) ? tabBlockedStats[tabId] : 0;
        return {
            ...
            pageBlockedCount,
            totalBlockedCount: typeof totalBlockedStats !== 'undefined' ? totalBlockedStats : 0,
        };
    }
