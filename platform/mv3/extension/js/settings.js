...
import { browser, i18n, runtime, sendMessage } from './ext.js';
...
async function backupSettings() {
    ...
    const appName = (manifest?.name || 'shieldblock')
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '') || 'shieldblock';
    const dateStr = new Date().toISOString().slice(0, 10);
    dom.attr(a, 'download', `${appName}-backup_${dateStr}.json`);
