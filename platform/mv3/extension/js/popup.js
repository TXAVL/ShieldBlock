...
function render() {
    ...
    dom.text('#hostname', /[\u0400-\u04ff]/.test(pretty) ? hostname : (pretty || 'Trang hệ thống'));
    dom.attr('#hostname', 'title', hostname || '');
    dom.text('#version', `v${runtime.getManifest().version}`);

    const favicon = qs$('#siteFavicon');
    if ( favicon ) {
    ...
    const ytCard = qs$('#ytCard');
    if ( ytCard ) {
        const isYT = Boolean(hostname && hostname.includes('youtube.com'));
        ytCard.style.display = isYT ? 'flex' : 'none';
        if ( isYT && Number.isInteger(currentTab.id) && typeof browser.tabs?.sendMessage === 'function' ) {
            browser.tabs.sendMessage(currentTab.id, { action: 'CHECK_YOUTUBE_AD_STATUS' }).then(res => {
                const badge = qs$('#ytBadgeStatus');
                const desc = qs$('#ytCardDesc');
                if ( res?.isAd ) {
                    if ( badge ) { badge.textContent = 'Phát hiện Ads'; badge.style.background = '#ef4444'; }
                    if ( desc ) desc.textContent = 'Đang có quảng cáo phát trong video. Nhấp để bỏ qua ngay!';
                } else {
                    if ( badge ) { badge.textContent = 'Video chính an toàn'; badge.style.background = '#22c55e'; }
                    if ( desc ) desc.textContent = 'Đang xem nội dung chính. Trình bảo vệ ngăn chặn việc tua nhầm.';
                }
            }).catch(() => {});
        }
    }

    const totalRulesetsEl = qs$('#totalRulesetsCount');
    if ( totalRulesetsEl ) {
        totalRulesetsEl.textContent = `${count ?? 56}`;
    }

    const appModeEl = qs$('#appModeBadge');
    if ( appModeEl ) {
        appModeEl.textContent = state.enabled ? 'Đang bảo vệ' : 'Đã tạm dừng';
    }

    // Cập nhật thống kê số ads bị chặn: trên trang này và tổng cộng
    let pageCount = popupPanelData.pageBlockedCount || 0;
    let totalCount = popupPanelData.totalBlockedCount || 0;

    const pageBlockedEl = qs$('#pageBlockedCount');
    if ( pageBlockedEl ) {
        pageBlockedEl.textContent = pageCount.toLocaleString();
    }
    const totalBlockedEl = qs$('#totalBlockedCount');
    if ( totalBlockedEl ) {
        totalBlockedEl.textContent = totalCount.toLocaleString();
    }

    if ( Number.isInteger(currentTab.id) && typeof browser.action?.getBadgeText === 'function' ) {
        browser.action.getBadgeText({ tabId: currentTab.id }).then(badge => {
            if ( badge && !isNaN(parseInt(badge, 10)) ) {
                const badgeNum = parseInt(badge, 10);
                if ( badgeNum > pageCount ) {
                    pageCount = badgeNum;
                    if ( pageCount > totalCount ) {
                        totalCount = pageCount;
                    }
                    if ( pageBlockedEl ) pageBlockedEl.textContent = pageCount.toLocaleString();
                    if ( totalBlockedEl ) totalBlockedEl.textContent = totalCount.toLocaleString();
                }
            }
        }).catch(() => {});
    }
...
function renderDetailsToggle(settings) {
    ...
    const label = qs$('#toggleDetails .adv-label');
    if ( label ) {
        label.textContent = expanded ? 'Thu gọn cài đặt' : 'Cài đặt nâng cao';
    } else {
...
dom.on('#ytSkipBtn', 'click', async () => {
    if ( !Number.isInteger(currentTab.id) ) return;
    const btn = qs$('#ytSkipBtn');
    const desc = qs$('#ytCardDesc');
    const badge = qs$('#ytBadgeStatus');
    const originalText = '⏭️ Bỏ qua Ads Video ngay';

    try {
        let handled = false;

        // 1. Thử gửi thông điệp tới youtube content script (nơi nắm rõ trạng thái DOM của player)
        try {
            if ( typeof browser.tabs?.sendMessage === 'function' ) {
                const res = await browser.tabs.sendMessage(currentTab.id, { action: 'SKIP_YOUTUBE_AD_NOW' });
                if ( res instanceof Object ) {
                    handled = true;
                    if ( res.success ) {
                        if ( btn ) {
                            btn.textContent = '⚡ Đã bỏ qua Ads thành công!';
                            btn.style.background = 'linear-gradient(135deg, #22c55e, #16a34a)';
                        }
                        if ( desc ) desc.textContent = 'Đã bỏ qua quảng cáo an toàn và trở lại nội dung video!';
                        if ( badge ) { badge.textContent = 'Đã bỏ qua'; badge.style.background = '#22c55e'; }
                    } else if ( res.reason === 'no_ad_playing' ) {
                        if ( btn ) {
                            btn.textContent = '🛡️ Video chính an toàn (Không tua nhầm)!';
                            btn.style.background = 'linear-gradient(135deg, #0284c7, #0369a1)';
                        }
                        if ( desc ) desc.textContent = 'Video chính đang phát bình thường, đã kích hoạt bảo vệ chống tua nhầm.';
                        if ( badge ) { badge.textContent = 'Video an toàn'; badge.style.background = '#0284c7'; }
                    }
                }
            }
        } catch {}

        // 2. Fallback dự phòng: executeScript với cơ chế BẢO VỆ TUYỆT ĐỐI
        if ( !handled && typeof browser.scripting?.executeScript === 'function' ) {
            const [ injectionResult ] = await browser.scripting.executeScript({
                target: { tabId: currentTab.id },
                func: () => {
                    const player = document.getElementById('movie_player') || document.querySelector('.html5-video-player');
                    const video = document.querySelector('video');
                    if ( !player || !video ) return { reason: 'no_player' };

                    // KIỂM TRA NGHIÊM NGẶT: Chỉ được phép can thiệp khi ĐÚNG LÀ ĐANG PHÁT QUẢNG CÁO
                    const isAd = player.classList.contains('ad-showing') ||
                                 player.classList.contains('ad-interrupting') ||
                                 Boolean(player.querySelector('.ytp-ad-player-overlay, .ytp-ad-text')) ||
                                 Boolean(document.querySelector('.ytp-ad-skip-button, .ytp-ad-skip-button-modern, .ytp-skip-ad-button'));

                    if ( !isAd ) {
                        // TUYỆT ĐỐI KHÔNG CHẠM VÀO currentTime CỦA VIDEO CHÍNH
                        return { reason: 'no_ad' };
                    }
            ...
            const r = injectionResult?.result;
            if ( r?.success ) {
                if ( btn ) {
                    btn.textContent = '⚡ Đã bỏ qua Ads thành công!';
                    btn.style.background = 'linear-gradient(135deg, #22c55e, #16a34a)';
                ...
                if ( desc ) desc.textContent = 'Video chính đang phát bình thường, không có quảng cáo nào.';
            }
        }
